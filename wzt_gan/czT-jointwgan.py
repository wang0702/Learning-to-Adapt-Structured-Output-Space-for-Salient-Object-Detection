...
def get_args():
    ...
    parser.add_argument("--clip-value", type=float, default=CLIP_VALUE,
                        help="Clip value")    
...
def main():
    ...
    optimizer.zero_grad()
    optimizer_D1 = optim.RMSprop(model_D1.parameters(), lr=args.learning_rate_D)
    optimizer_D1.zero_grad() 
    optimizer_D2 = optim.RMSprop(model_D2.parameters(), lr=args.learning_rate_D)
    ...
    for i_epotch in range(args.epotch_size):
        ...
        for i_iter, data_batch in enumerate(picloader):
            ...
            loss_adv_target1 = torch.mean(model_D1(s_edge_pred)) # 后面一个相当于全部是正确答案的和前一个size相同的tensor
            ...
            loss_adv_target2 = -torch.mean(model_D2(e_sal_pred[0])) # 后面一个相当于全部是正确答案的和前一个size相同的tensor
            for ix in e_sal_pred[1]:
                loss_adv_target2 += -torch.mean(model_D2(ix))
            ed_loss = loss_adv_target2 / (args.iter_size * args.batch_size) / (len(e_sal_pred[1]) + 1)
            ...
            es_Loss = es_loss / (args.iter_size * args.batch_size) / (len(e_sal_pred[1]) + 1)
            ...
            ee_Loss = ee_loss / (args.iter_size * args.batch_size) / (len(e_edge_pred[1]) + 1)
        ...
        if i_epotch == 7:
            ...
            optimizer = optim.RMSprop(filter(lambda p: p.requires_grad, model.parameters()), lr=args.learning_rate, weight_decay=args.weight_decay)
            optimizer_D1 = optim.RMSprop(model_D1.parameters(), lr=args.learning_rate_D)
            optimizer_D2 = optim.RMSprop(model_D2.parameters(), lr=args.learning_rate_D)                        
