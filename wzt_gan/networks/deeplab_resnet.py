...
class ResNet_locate(nn.Module):
    ...
    def forward(self, x):
        ...
        for k in range(len(self.ppms)):
            xls.append(F.interpolate(self.ppms[k](xs_1), xs_1.size()[2:], mode='bilinear', align_corners=True)) # 放入模型得到全局参数并稀释为xs_1的size
        xls = self.ppm_cat(torch.cat(xls, dim=1)) # 接一起
        
        # 注意这个是倒着的
        infos = []
        for k in range(len(self.infos)):
            infos.append(self.infos[k](F.interpolate(xls, xs[len(self.infos) - 1 - k].size()[2:], mode='bilinear', align_corners=True))) # 先把xls的size变为指定中间卷积层的size并进行一次经典33卷积

        return xs, infos # res的中间变量，做好的ppm变量
